...
/**
 * Reports health of the SIONA connector.
 *
 * Always returns HTTP 200. Use the "status" field in the body to determine
 * health state. This allows upstream monitors to distinguish unconfigured
 * (expected in dev) from degraded/error states without treating 503 as a
 * fatal alerting condition during normal setup.
 *
 * Security invariants:
 * - SIONA_API_TOKEN is never included in the response.
 * - Raw exceptions are caught and summarised before reaching the browser.
 */
class SionaHealthController extends Controller
{
    ...
    private function probeHealth(string $apiUrl): JsonResponse
    {
        ...
        $probeUrl  = rtrim($apiUrl, '/') . '/api/health';
    ...
    {
        // Strip anything that looks like a URL with auth credentials or token values
