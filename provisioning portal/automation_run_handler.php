...
require_once __DIR__ . '/auth/bootstrap.php'; // csrf_verify()
...
if ($initiatedVia === '') $initiatedVia = 'manual';
...
if ($targetMode === 'single') {
  ...
  // multi mode requires at least one targeting method
  if (count($targets['site_groups']) === 0 && count($targets['provider_groups']) === 0 && count($targets['node_ids']) === 0) {
    http_response_code(400);
...
// Resolve script
$resolved = [
  'script_source' => null,   // saved|custom
  'script_id' => null,
  'command' => null,
...
if ($scriptId === '__custom__') {
  if (!$canCustomScript) {
    http_response_code(403);
    exit('Custom scripts restricted to owner/admin');
  }
  if ($customCmd === '') {
    http_response_code(400);
    exit('Custom script cannot be empty');
  }
  $resolved['script_source'] = 'custom';
  $resolved['command'] = $customCmd;

  // Optionally save custom script
  if ($saveCustom) {
    if ($customName === '') {
      http_response_code(400);
      exit('Provide a name to save the script');
    }
    $stmt = $pdo->prepare(
  ...
  // saved script selection required
  $sid = (int)$scriptId;
  if ($sid <= 0) {
    http_response_code(400);
    exit('Select a saved script (or custom)');
  }

  $stmt = $pdo->prepare("SELECT id, command FROM ansible_scripts WHERE id = ? AND is_active=1 LIMIT 1");
  $stmt->execute([$sid]);
  $row = $stmt->fetch();

  if (!$row) {
    http_response_code(400);
    exit('Saved script not found or inactive');
  }
...
// Map to an automation record (simple approach)
// We’ll use/ensure a single “Run Ansible” automation exists so runs can join to it.
$automationId = null;
...
// Log a few structured events
$logStmt = $pdo->prepare(
  "INSERT INTO automation_run_logs (run_id, level, message, context)
   VALUES (?, ?, ?, ?)"
);

$logStmt->execute([$runId, 'info', 'Run created (queued)', json_encode(['meta' => $meta], JSON_UNESCAPED_SLASHES)]);
$logStmt->execute([$runId, 'info', 'Targets resolved', json_encode($targets, JSON_UNESCAPED_SLASHES)]);
$logStmt->execute([$runId, 'info', 'Script selected', json_encode(['source'=>$resolved['script_source'],'script_id'=>$resolved['script_id'],'saved_new_script_id'=>$resolved['saved_new_script_id']], JSON_UNESCAPED_SLASHES)]);
...
$aStmt->execute([
  ...
  $ip,
  $ua,
  json_encode(['automation_id'=>$automationId,'run_id'=>$runId], JSON_UNESCAPED_SLASHES),
]);
